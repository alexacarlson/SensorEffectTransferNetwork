...
def aug_chromab(image, crop_h, crop_w, scale_val, tx_Rval, ty_Rval, tx_Gval, ty_Gval, tx_Bval, ty_Bval):
    ...
    augR = perform_aff_transformation(R, HRt, (crop_h, crop_w, 1))
    augG = perform_aff_transformation(G, HGt, (crop_h, crop_w, 1))
    augB = perform_aff_transformation(B, HBt, (crop_h, crop_w, 1))
    #
...
def aug_color(image_rgb, a_transl, b_transl):
    ...
    auglab_ = tf.concat([Lchan, aug_a, aug_b], axis=3)
    augim_rgb = lab_to_rgb(auglab_)
    # clip
    augim_rgb = tf.clip_by_value(augim_rgb,0.0,1.0)
...
### ---------------------------------------------------------------- ###
def aug_noise_mask(image_rgb, noise_mask, batchsize, im_h, im_w):
    #
    ...
    Rcfa, Gcfa, Bcfa, rt, gt, bt = return_bayer(bayer_type, im_h, im_w, batchsize) 
    ## define cfa interpolation kernels
    Rcfa_kernel = 0.25*np.array([[1,2,1],[2,4,2],[1,2,1]])
    Bcfa_kernel = 0.25*np.array([[1,2,1],[2,4,2],[1,2,1]])
    Gcfa_kernel = 0.25*np.array([[0,1,0],[1,4,1],[0,1,0]])
    nd=3
    Rcfa_kernel = tf.constant(Rcfa_kernel, dtype=tf.float32, shape = (nd,nd,1,1))
    Gcfa_kernel = tf.constant(Gcfa_kernel, dtype=tf.float32, shape = (nd,nd,1,1))
    Bcfa_kernel = tf.constant(Bcfa_kernel, dtype=tf.float32, shape = (nd,nd,1,1))
    ##
    ## normalize images
    ...
    Rmaskdi, Gmaskdi, Bmaski, Rmaskd, Gmaskd, Bmaskd = tf.split(noise_mask, 6, axis=3)
    #
    ## add in realistic (poisson gaussian) sensor noise to each channel ##
    Rchan_ = tf.sqrt(Rchan + Rchan*Rmaskdi + Rmaskd)*tf.random_normal(shape=(batchsize,im_h, im_w, 1), mean=0.0, stddev=1.0)
    Gchan_ = tf.sqrt(Gchan + Gchan*Gmaskdi + Gmaskd)*tf.random_normal(shape=(batchsize,im_h, im_w, 1), mean=0.0, stddev=1.0)
    Bchan_ = tf.sqrt(Rchan + Gchan*Gmaskdi + Bmaskd)*tf.random_normal(shape=(batchsize,im_h, im_w, 1), mean=0.0, stddev=1.0)
    #
    ## add in effects from bilinear interpolation on bayer cfa ##
    #Rchan_ = bilinear_interp_cfa(Rchan_, Rcfa, Rcfa_kernel, batchsize)
    #Gchan_ = bilinear_interp_cfa(Gchan_, Gcfa, Gcfa_kernel, batchsize)
    #Bchan_ = bilinear_interp_cfa(Bchan_, Bcfa, Bcfa_kernel, batchsize)
    #
    ## compose the noisy image:
    #augnoise = tf.concat([Rchan_,Gchan_,Bchan_],axis=3) 
    augnoise = tf.concat([Rchan_,Gchan_,Bchan_],axis=3) 
...
def aug_noise(image_rgb, batchsize, Ra_sd, Rb_si, Ga_sd, Gb_si, Ba_sd, Bb_si, im_h, im_w):
    ...
    #
    ## add in realistic (poisson gaussian) sensor noise to each channel ##
    #Rchan_ = Rchan + tf.sqrt(Ra_sd*Rchan + Rb_si)*tf.random_normal(shape=(batchsize,im_h, im_w, 1), mean=0.0, stddev=1.0)
    Rchan_ = Rchan + (Ra_sd*Rchan + Rb_si)*tf.random_normal(shape=(batchsize,im_h, im_w, 1), mean=0.0, stddev=1.0)
    #Gchan_ = Gchan + tf.sqrt(Ga_sd*Gchan + Gb_si)*tf.random_normal(shape=(batchsize,im_h, im_w, 1), mean=0.0, stddev=1.0)
    Gchan_ = Gchan + (Ga_sd*Gchan + Gb_si)*tf.random_normal(shape=(batchsize,im_h, im_w, 1), mean=0.0, stddev=1.0)
    #Bchan_ = Bchan + tf.sqrt(Ba_sd*Bchan + Bb_si)*tf.random_normal(shape=(batchsize,im_h, im_w, 1), mean=0.0, stddev=1.0)
    Bchan_ = Bchan + (Ba_sd*Bchan + Bb_si)*tf.random_normal(shape=(batchsize,im_h, im_w, 1), mean=0.0, stddev=1.0)
    #
    ## add in effects from bilinear interpolation on bayer cfa ##
    Rchan_ = bilinear_interp_cfa(Rchan_, Rcfa, Rcfa_kernel, batchsize)
    Gchan_ = bilinear_interp_cfa(Gchan_, Gcfa, Gcfa_kernel, batchsize)
    ...
    #augnoise = tf.concat([Rchan_,Gchan_,Bchan_],axis=3) 
    augnoise = tf.concat([Rchan_,Gchan_,Bchan_],axis=3) 
    augnoise = tf.clip_by_value(augnoise, 0.0, 1.0)
    ## scale image to 0-255
    augimg = tf.multiply(augnoise, 255.0)
    #pdb.set_trace()
    return augimg
    #
...
def add_channel_noise(chan, a_sd, b_si, batchsize, im_h, im_w):
    ...
    chan_with_noise = sigdep + sigindep
    # clip the noise between 0 and 1 (baking in 0 and 255 limits)
    clip_chan_with_noise = tf.clip_by_value(chan_with_noise, 0.0, 1.0)
    #
    return clip_chan_with_noise

def bilinear_interp_cfa2(chan, cfa_kernel,batchsize):
    #
    ...
    interp_pixs = tf.nn.conv2d(chan, cfa_kernel, strides=[1, 1, 1, 1], padding='SAME')
    #
    #pdb.set_trace()
    return interp_pixs

def bilinear_interp_cfa(chan, cfa, cfa_kernel,batchsize):
    #
    # calculate pixel intensities based upon bayer CFA pattern
    #
...
def return_bayer(bayer_type, im_h, im_w, batchsize):
    ...
    #
    return Rcfa, Gcfa, Bcfa, Cr, Cg, Cb 
...
def aug_blur(img_inp, window_l, sig_arr, batchsize):
    ...
    for bimg in batch_list:
        ...
        # conv
        bimg_conv = tf.nn.conv2d(bimg, fgauss, strides=[ 1, 1, 1, 1], padding='SAME')
        conv_batch_list.append(bimg_conv)
        batch_counter+=1
        #
    #img_conv= tf.stack(conv_batch_list,0)
    #augimage = tf.squeeze(img_conv)
    #augimage = tf.stack(conv_batch_list,0)
    augimage = tf.concat(conv_batch_list,axis=0)
    # clip
    augimage = tf.clip_by_value(augimage,0.0,1.0)
    # scale image back into 0-255 range
    augimage = tf.multiply(augimage,255.0)
...
def aug_blur2(img_inp, window_l, sig_arr, batchsize):
    #
    # Blur
    # Iprime = cv2.GaussianBlur(image,(hsize,hsize),sigma)
    #
    ...
    for bimg in batch_list:
        ...
        bimg_conv= tf.concat([R_conv, G_conv, B_conv], axis=3)
        conv_batch_list.append(bimg_conv)
        batch_counter+=1
        #
    augimage = tf.concat(conv_batch_list,axis=0)
...
def gaussiankern2D(wl, sig):
    ...
    #expkernel_ = tf.tile(expkernel_,[1,1,3,3])
    return expkernel_
