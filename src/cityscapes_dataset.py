...
class CityscapesDataset:
    ...
    def __init__(self, paths, *, cache_dir, calc_val_mean):
        """
...
def load_dataset(path, max_train_images=None, slug=None):
    """
    Loads a dataset from a directory according to conventions of cityscapes.
    ...
    def load_subset(subset, limit=None,):
        ...
        images = sorted(glob.glob('{}/*.png'.format(images_subset_dir)))
        labels = sorted(glob.glob('{}/*.png'.format(labels_subset_dir)))
        #
...
def merge_datasets(datasets):
    def merge_subsets(subsets):
        return {
            'images': sum([s['images'] for s in subsets], []),
            'labels': sum([s['labels'] for s in subsets], [])
        }
    return {
